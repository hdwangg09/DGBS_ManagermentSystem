...
import { getListKetQuaDauGia } from "../../../services/public/daugia/bienSoServices";
import { Spin } from "antd";

function KetQua() {
    const [loading, setLoading] = useState(true);
    const [ketQuaDauGiaData, setKetQuaDauGiaData] = useState([]);
    const [searchText, setSearchText] = useState("");
    const [searchedColumn, setSearchedColumn] = useState("");
    const searchInput = useRef(null);

    useEffect(() => {
        getKetQuaDauGiaInfor();
    }, []);

    const getKetQuaDauGiaInfor = async () => {
        setLoading(true);
        try {
            const res = await getListKetQuaDauGia();
            console.log(res)
            if (res && res.code === 200 && res.data) {
                setKetQuaDauGiaData(res.data);
            }
        } catch (error) {
            console.log(error)
        }
    ...
    const columns = [
        ...
        },
        {
            ...
            dataIndex: "loaiXe",
            width: 100,
            ...getColumnSearchProps("loaiXe"),
            render: (text, record) => <p>{text}</p>,
        },
        {
            title: "Người Thắng Cuộc",
            dataIndex: "nguoiThangCuoc",
            width: 100,
            ...getColumnSearchProps("nguoiThangCuoc"),
            render: (text, record) => <p>{text}</p>,
        },
        {
            title: "Giá cuối cùng",
            dataIndex: "giaCuoiCung",
            width: 100,
            sorter: {
                compare: (a, b) => a.giaCuoiCung - b.giaCuoiCung,
                multiple: 2,
            },
            ...getColumnSearchProps("giaCuoiCung"),
